...
def VideoToFrames(video):
    ...
    if CountImages() > 0:
        print "\nTemp file already exists, using it to relieve overhead\n---------\n\n\n\n\n"
        return
...
def BuildVideo(images, fps):
    SaveImages(images)
    command = "ffmpeg -framerate {} -i %03d.bmp output.avi".format(fps)
    call(command, shell=True)


def SaveEncoding(first_matrix, diff, Q, fps, w, h, frames_count):
    diff = diff.astype(np.int16)
    d = diff.flatten()
    f = first_matrix.flatten().astype(np.int16)

    toWrite = np.array([w, h, fps, Q, frames_count]).astype(np.int16)
    toWrite = np.concatenate([toWrite, f])
    toWrite = np.concatenate([toWrite, d])
